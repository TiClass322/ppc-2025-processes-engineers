...
bool KopilovDSumValColMatMPI::RunImpl() {
    ...
    MPI_Scatterv(sendBufferPtr, sendCounts.data(), displacements.data(), MPI_DOUBLE,
                 recvBuffer.data(), recvCount, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    ...
    for (int row = 0; row < localRows; ++row) {
        for (int col = 0; col < cols; ++col) {
            localColSum[static_cast<std::size_t>(col)] += recvBuffer[static_cast<std::size_t>(row) * static_cast<std::size_t>(cols) + static_cast<std::size_t>(col)];
        }
    }
